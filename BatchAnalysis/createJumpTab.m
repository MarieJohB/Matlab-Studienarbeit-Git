function createJumpTab(tab, batchResults)
    % CREATEJUMPTAB - Creates zero steady-state error analysis plots directly on the tab
    ...
    errorAxes = uiaxes(tab, 'Position', [50 410 1800 270]);
    ...
    htmlContent = ['<html><head><style>', ...
        ...
        '</style></head><body>', ...
        '<table>', ...
        '<tr class="header"><th colspan="2">Zero Steady-State Error Analysis Summary</th></tr>'];
    ...
    if isempty(zeroSteadyStateRanges)
        ...
        for i = 1:size(zeroSteadyStateRanges, 1)
            ...
            rangeStr = [rangeStr, sprintf('Range %d: %.6f - %.6f', i, zeroSteadyStateRanges(i, 1), zeroSteadyStateRanges(i, 2))];
        end
